...
def genetic_algorithm(im, nPopulation, nGenerations):
    ...
    while nGenerations != cGeneration:
        scores = sorted([(key, item["score"] / 1440) for key, item in im.iteration_dct.items()], key=itemgetter(1))
        ...
        while parents:
            ...
            father_lectures = [im.iteration_dct[father][x] for x in range(0, intersect)]
            mother_lectures = [im.iteration_dct[mother][x] for x in range(intersect, nLectures)]
            
            child_lectures = {i: x for i, x in enumerate(father_lectures + mother_lectures)}
    ...
    im.exportLectures("GA%sp%sg%s" % (best_iteration[1], nPopulation, nGenerations))
